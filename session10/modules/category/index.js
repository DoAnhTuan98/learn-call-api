...
const handlers = {
    async findMany(req, res, next) {
        try {
            ...
            let items = await categoryModel.find(conditions, fieldsArr).skip(skip).limit(limit).sort(sortInfo)
            res.json(items)
        } catch (err) {
            next(err)
        }
    ...
    async findOne(req, res, next) {
        try {
            ...
            let item = await categoryModel.findOne(id)
            res.json(item)
        } catch (err) {
            next(err)
        }
    ...
    async delete(req, res, next) {
        try {
            let id = req.params.id
            let item = await findByIdAndDelete(id)
            res.json(item)
        } catch (err) {
            next(err)
        }
